...
import {
	ExcelSheetReferenceParameter,
	ReferenceParameters,
	SpreadedReferenceParameter,
	Indicator,
} from "./shared/referenceParameter";
import { Display } from "./shared/display";
import path = require("path");

let flow = argv.q;
let dischargePressure = argv.pd;
let current = argv.i;
let efficiency = argv.n;
let amplitudeOfVibration = argv.lemda;
let vibrationSpikeEnergy = argv.se;
let pumpTemperature = argv.tp;
let bearingTemperature = argv.tb;
let controlValveOpening = argv.cv;
let csvFilePath = argv.file;
let defaultFilePath = "assets/devation_data.csv";

let fetchedData: ExcelSheetReferenceParameter[] = [];

let referenceDataArray: ReferenceParameters[] = [];
...
function onReferenceDataFetchComplete() {
	let referenceData: ReferenceParameters;
	for (let i = 0; i < fetchedData.length; i = i + 6) {
		referenceData = {
			...
			failureCauseDescription:
				fetchedData[i]["Possible cause of the failure (Failure Mode)"],
			flow: {
			...
			changeInVibration:
				fetchedData[i]["Change in Vibration"] == "yes" ? true : false,

			amplitudeOfVibration: {
				indicator: fetchedData[i]["Vib. Signature, l (mm/s)"],
				setValue: Number(
					fetchedData[i + 1]["Vib. Signature, l (mm/s)"]
				),
				thresholdValue1: Number(
					fetchedData[i + 2]["Vib. Signature, l (mm/s)"]
				),
				deviation1: Number(
					fetchedData[i + 3]["Vib. Signature, l (mm/s)"]
				),
				thresholdValue2: Number(
					fetchedData[i + 4]["Vib. Signature, l (mm/s)"]
				),
				deviation2: Number(
					fetchedData[i + 5]["Vib. Signature, l (mm/s)"]
				),
			},
			vibrationSpikeEnergy: {
				...
				thresholdValue1: Number(
					fetchedData[i + 2]["Spike energy, gSE"]
				),
				deviation1: Number(fetchedData[i + 3]["Spike energy, gSE"]),
				thresholdValue2: Number(
					fetchedData[i + 4]["Spike energy, gSE"]
				),
			...
			controlValveOpening: {
				...
				setValue: Number(
					fetchedData[i + 1]["Disc. Cntrl. Vlv. Opening (%)"]
				),
				thresholdValue1: Number(
					fetchedData[i + 2]["Disc. Cntrl. Vlv. Opening (%)"]
				),
				deviation1: Number(
					fetchedData[i + 3]["Disc. Cntrl. Vlv. Opening (%)"]
				),
				thresholdValue2: Number(
					fetchedData[i + 4]["Disc. Cntrl. Vlv. Opening (%)"]
				),
				deviation2: Number(
					fetchedData[i + 5]["Disc. Cntrl. Vlv. Opening (%)"]
				),
			},
			averageDeviation: {
				averageDeviation1: Number(
					fetchedData[i + 2]["Average Deviation"]
				),
				averageDeviation2: Number(
					fetchedData[i + 4]["Average Deviation"]
				),
			},
		};
...
function onReferenceDataObjectCreation() {
	...
	referenceDataArray.forEach((referenceData) => {
		let averageDeviation = getAverageDeviation(referenceData);
		if (
			averageDeviation >=
				referenceData.averageDeviation.averageDeviation1 &&
			averageDeviation < referenceData.averageDeviation.averageDeviation2
		) {
			displayMessage(referenceData, Display.warning);
		} else if (
			averageDeviation >= referenceData.averageDeviation.averageDeviation2
		) {
			displayMessage(referenceData, Display.error);
		}
	});
...
function displayMessage(
	referenceData: ReferenceParameters,
	messageType: string
) {
	const screenWidth = process.stdout.columns;
	let upperLine: string = "";
	let lowerLine: string = "";
	for (let i = 0; i < screenWidth / 2; i++) {
		if (i > 4) upperLine = upperLine + "-";
		lowerLine = lowerLine + "-";
	}
	...
	if (messageType == Display.warning) totalWarnings++;
	else if (messageType == Display.error) totalErrors++;
}

function getCalc(
	spreadedReferenceParamters: SpreadedReferenceParameter,
	inputParameterValue: number,
	flag: boolean = false
): number {
	let calc: number = 0;
	if (
		(spreadedReferenceParamters.indicator != Indicator.noChange &&
			(spreadedReferenceParamters.indicator == Indicator.increament ||
				spreadedReferenceParamters.indicator == Indicator.decreament ||
				spreadedReferenceParamters.indicator ==
					Indicator.smallIncreament ||
				spreadedReferenceParamters.indicator ==
					Indicator.smallDecreament ||
				spreadedReferenceParamters.indicator ==
					Indicator.largeIncreament ||
				spreadedReferenceParamters.indicator ==
					Indicator.largeDecreament)) ||
		flag
	) {
		if (inputParameterValue)
			calc = Math.abs(
				(spreadedReferenceParamters.setValue - inputParameterValue) /
...
function getAverageDeviation(referenceData: ReferenceParameters): number {
	changeCount = 0;
	let calcFlow: number = getCalc(referenceData.flow, flow);
	let calcDischargePressure: number = getCalc(
		referenceData.dischargePressure,
		dischargePressure
	);
	let calcCurrent: number = getCalc(referenceData.current, current);
	let calcEfficiency: number = getCalc(referenceData.efficiency, efficiency);
	let calcAmplitudeOfVibration: number = getCalc(
		referenceData.amplitudeOfVibration,
		amplitudeOfVibration,
		true
	);
	let calcVibrationSpikeEnergy: number = getCalc(
		referenceData.vibrationSpikeEnergy,
		vibrationSpikeEnergy
	);
	let calcControlValveOpening: number = getCalc(
		referenceData.controlValveOpening,
		controlValveOpening
	);
	let calcPumpTemperature: number = getCalc(
		referenceData.pumpTemperature,
		pumpTemperature
	);
	let calcBearingTemperature: number = getCalc(
		referenceData.bearingTemperature,
		bearingTemperature
	);
	...
	let averageDeviation = sum / changeCount;
